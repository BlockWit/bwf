...
@Slf4j
@Service
public class PostService implements IPageableService<Post> {
	...
	@Override
	public Page<Post> findPageable(Pageable pageable) {
		...
		return new PageImpl(StreamEx.of(content)
			.map(post -> post.toBuilder()
				.owner(StreamEx.of(accounts).findFirst(t -> t.getId().equals(post.getOwnerId())).get())
				.build())
			.toList(),
			page.getPageable(),
			page.getTotalElements());
